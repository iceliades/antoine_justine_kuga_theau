...
//Do the setters do their job ?
            ...
            Crook.setName("newCrook");
            Crook.setMovement(5);
            BOOST_CHECK_EQUAL(Crook.getName(), "newCrook");
            BOOST_CHECK_EQUAL(Crook.getMovement(), 5);
            ...
            Position pos1{px - 1, py};
            Position pos2{px + 1, py};
            Position pos3{px, py + 1};
            Position pos4{px, py - 1};
            allowpos.push_back(pos1);
            allowpos.push_back(pos2);
            allowpos.push_back(pos3);
            allowpos.push_back(pos4);
            ...
            BOOST_CHECK_EQUAL(
                    myAllowState.getListCharacters(0)[0]->allowedAttackPos(myAllowState)[0].equals(allowpos[0]), true);

            std::vector<int> targetallow;
            targetallow.push_back(0);
...
//------------------------------------------------------ Weapon --------------------------------------------------------
    ...
    } // ending bracket for the test case

} // ending bracket for the namespace